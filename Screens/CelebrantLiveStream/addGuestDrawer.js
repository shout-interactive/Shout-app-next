...
import { Box, SwipeableDrawer, Stack, Badge, Paper, Typography, IconButton, InputBase, Button, Avatar, Radio } from "@mui/material";
...
const AddGuestDrawer = ({ show, toggleDrawer }) => {
		...
		<Box sx={{ width: "auto" }} role="presentation">
			<Box className={classes.bottomDrawerWrapper}>
				...
				<Box sx={{ margin: "1.5rem 0rem" }} className={classes.listItem}>
					<Stack direction="horizontal" gap={3} className={classes.listItemStack}>
						<Avatar sx={{ bgcolor: "purple" }} variant="rounded">
							<CampaignIcon />
						</Avatar>

						<Box sx={{ width: "90%" }}>
							<Typography sx={{ color: "#0A1F44", fontWeight: "bold" }}>Invite from contacts</Typography>
							<Typography variant="p" sx={{ color: "#818FA3", fontSize: ".8rem" }}>
				...
				<List className={classes.listWrapper}>
					{dummyFriendsData.map((data) => (
						<ListItem onClick={() => handleSelectFriend(data.key)} className={classes.listItem} button key={data.key}>
							<Stack direction="horizontal" gap={1} sx={{  width: "100%", display: "flex", flexDirection: "row", alignItems: "center"}}>
				...
				<Box onClick={() => handleAddFriend()} sx={{ marginTop: ".5rem" }} >
					<Button sx={{ backgroundColor: selectedIndexArr.length > 0 ? "#3CC13B" : "#818FA3", textTransform: "capitalize", padding: ".8rem 0rem", borderRadius: "10px", fontWeight: "bold" }} variant="contained" fullWidth>
						Add
					</Button>
				</Box>
			</Box>
