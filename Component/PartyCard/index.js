      ...
      <Box sx={{ width: "100%" }}>
        <CardContent className={`${classes.squareContent}`}>
          <Stack className={classes.stack} direction="horizontal" gap={2}>
            <div className={`badge ${classes.badge}`}>
              Party start on{" "}
          ...
          <Box className={classes.partyCardTitleWrapper}>
            <h4 className={`title ${classes.partyCardTitle}`}>
          ...
          <div className={classes.avatarGroupContainer}>
            <AvatarGroup
              total={
                Number(
                  data?.Geust?.invites.length + data?.Geust?.geusts.length
                ) || 20
              }
            >
          ...
          <div className={`btn-box ${classes.buttonContainer}`} id={enterParty}>
            <ButtonComponent
              handleClick={partyBtnFunction}
              title="Enter Party"
              button={button}
            />
          </div>
...
export const PartyCardTwo = ({ secondary, header, onClick, id }) => {
  ...
  return (
    <Card className={classes.squareBox}>
      <CardContent
        className={[classes.squareContent, classes.squareBoxWrapper]}
      >
        ...
        <Box className={classes.partyCardTitleWrapper}>
          <h4 className={classes.partyCardTitle}>
            {/* {partyDetails?.name || "Anybody Cole"} */}
          </h4>
        </Box>
